...
function ApplyJob() {
  ...
  const onPhoneNumberChange = (event) => {
    if (
      !isValidUserPhone(event.target.value) ||
  ...
  const handleApply = (event) => {
    ...
    axios
      .post("http://localhost:4000/candidateDetails", { ...candidateDetails })
      .then((res) => {
        console.log(res);
      });
    if (
          ...
          <div class="contact-box">
            <div class="contact-left">
              ...
              <form onSubmit={handleApply}>
                ...
                {errorApply ? (
                  <div className="text-danger-apply">{errorApply}</div>
                ) : null}
                {successApply ? (
                  <Modal show={show} onHide={handleClose} animation={false}>
                    <Modal.Header>
                      <Modal.Title className="successMsg">
                        You have successfully applied
                      </Modal.Title>
                    </Modal.Header>
                    <Modal.Body className="successMsg">
                      Thank you for applying to our company
                    </Modal.Body>
                    <Modal.Footer>
                      <Button
                        variant="primary"
                        className="buttonCloseApply"
                        onClick={handleClose}
                      >
